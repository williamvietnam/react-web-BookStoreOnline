...
import { UPDATE_USER_ADDRESS, GET_PROVINCES, GET_DISTRICTS, GET_WARDS } from '../api/userAddressApi';
...
function EditAddressScreen(props) {
    ...
    return (
        <View style={styles.container}>
            <HeaderBackAction title="Cập nhật địa chỉ" />
            ...
            <View style={{ paddingHorizontal: 12, paddingVertical: 8, backgroundColor: '#fff' }}>
                <Button onPress={() => updateUserAddress({
                    variables: {
                        id: inputs.id,
                        data: {
                    ...
                    title="LƯU ĐỊA CHỈ" loading={updatingUserAddress} />
            </View>
        </View>
    )
